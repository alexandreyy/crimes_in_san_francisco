...
if (! exists("workspaceDirectory")) {
  # Workspace isn't setted.
  
  # Set workspace.
  workspaceDirectory <- dirname(sys.frame(1)$ofile)
  setwd(workspaceDirectory)
} else {
  # Workspace is setted.
  
  # Clean workspace.
  rm(list = ls())
  workspaceDirectory <- getwd()
}
...
train <- read_csv("resources/train.csv.zip")
#map <- readRDS("../resources/sf_map_copyright_openstreetmap_contributors.rds")
